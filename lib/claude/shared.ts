...
// extractions. The input-format and layout descriptions must read identically
// in every prompt that mentions them; anything skill-specific stays in
// reading.ts / listening.ts.

// The two skills whose questions are digitised (Writing has none, Speaking is
// out of scope).
export type QuestionSkill = "Reading" | "Listening";

// What the extracted text looks like. Must say artefacts are to be REMOVED,
// not merely disregarded — "ignore" once read as "don't be confused by them",
// and every " | " was copied through.
...
// How the questions should come back: printed layout restored, not just words.
...
// Whether a page's text plainly carries questions — used to tell a page that
// legitimately has none from one where an empty answer is the model slipping,
// so only the latter is retried.
